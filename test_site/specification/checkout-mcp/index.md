...
This document specifies the Model Context Protocol (MCP) binding for the [Checkout Capability](https://ucp.dev/specification/checkout/index.md).
...
The `meta["ucp-agent"]` field is **required** on all requests to enable [capability negotiation](https://ucp.dev/specification/overview/#negotiation-protocol). The `complete_checkout` and `cancel_checkout` operations also require `meta["idempotency-key"]` for retry safety. Platforms **MAY** include additional metadata fields.
...
| ------------------- | ------------------------------------------------------------------------------ | -------------------------- |
| `create_checkout`   | [Create Checkout](https://ucp.dev/specification/checkout/#create-checkout)     | Create a checkout session. |
| `get_checkout`      | [Get Checkout](https://ucp.dev/specification/checkout/#get-checkout)           | Get a checkout session.    |
| `update_checkout`   | [Update Checkout](https://ucp.dev/specification/checkout/#update-checkout)     | Update a checkout session. |
| `complete_checkout` | [Complete Checkout](https://ucp.dev/specification/checkout/#complete-checkout) | Place the order.           |
| `cancel_checkout`   | [Cancel Checkout](https://ucp.dev/specification/checkout/#cancel-checkout)     | Cancel a checkout session. |

### `create_checkout`

Maps to the [Create Checkout](https://ucp.dev/specification/checkout/#create-checkout) operation.
...
Maps to the [Get Checkout](https://ucp.dev/specification/checkout/#get-checkout) operation.
...
- [Checkout](https://ucp.dev/specification/checkout/#get-checkout) object.

### `update_checkout`

Maps to the [Update Checkout](https://ucp.dev/specification/checkout/#update-checkout) operation.
...
Maps to the [Complete Checkout](https://ucp.dev/specification/checkout/#complete-checkout) operation.
...
- [Checkout](https://ucp.dev/specification/checkout/#complete-checkout) object, containing a partial `order` that holds only `id` and `permalink_url`.

### `cancel_checkout`

Maps to the [Cancel Checkout](https://ucp.dev/specification/checkout/#cancel-checkout) operation.
...
- [Checkout](https://ucp.dev/specification/checkout/#cancel-checkout) object with `status: canceled`.

## Error Handling

UCP distinguishes between protocol errors and business outcomes. See the [Core Specification](https://ucp.dev/specification/overview/#error-handling) for the complete error code registry and transport binding examples.
...
Platforms **SHOULD** authenticate agents when using MCP transport. When using HTTP Message Signatures, all checkout operations follow the [Message Signatures](https://ucp.dev/specification/signatures/index.md) specification.
...
See [Message Signatures - MCP Transport](https://ucp.dev/specification/signatures/#mcp-transport) for details.
...
See [Message Signatures - REST Response Signing](https://ucp.dev/specification/signatures/#rest-response-signing) for the signing algorithm (identical for MCP over HTTP).
...
1. Return errors per the [Core Specification](https://ucp.dev/specification/overview/#error-handling).
...
1. Authenticate agents using one of the supported mechanisms (API keys, OAuth, mTLS, or HTTP Message Signatures per [Message Signatures](https://ucp.dev/specification/signatures/index.md)).
