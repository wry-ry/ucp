...
The Cart capability enables basket building without the complexity of checkout. While [Checkout](https://ucp.dev/specification/checkout/index.md) manages payment handlers, status lifecycle, and order finalization, cart provides a lightweight CRUD interface for item collection before purchase intent is established.
...
| ------------ | ------------------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| ucp          | [UCP Response Cart Schema](/specification/cart/#ucp-response-cart-schema) | **Yes**  | Protocol metadata for discovery profiles and responses. Uses slim schema pattern with context-specific required fields.                            |
| id           | string                                                                    | **Yes**  | Unique cart identifier.                                                                                                                            |
| line_items   | Array\[[Line Item Response](/specification/cart/#line-item-response)\]    | **Yes**  | Cart line items. Same structure as checkout. Full replacement on update.                                                                           |
| context      | [Context](/specification/cart/#context)                                   | No       | Buyer signals for localization (country, region, postal_code). Merchant uses for pricing, availability, currency. Falls back to geo-IP if omitted. |
| buyer        | [Buyer](/specification/cart/#buyer)                                       | No       | Optional buyer information for personalized estimates.                                                                                             |
| currency     | string                                                                    | **Yes**  | ISO 4217 currency code. Determined by merchant based on context or geo-IP.                                                                         |
| totals       | Array\[[Total Response](/specification/cart/#total-response)\]            | **Yes**  | Estimated cost breakdown. May be partial if shipping/tax not yet calculable.                                                                       |
| messages     | Array\[[Message](/specification/cart/#message)\]                          | No       | Validation messages, warnings, or informational notices.                                                                                           |
| links        | Array\[[Link](/specification/cart/#link)\]                                | No       | Optional merchant links (policies, FAQs).                                                                                                          |
...
- [REST Binding](https://ucp.dev/specification/cart-rest/#create-cart)
- [MCP Binding](https://ucp.dev/specification/cart-mcp/#create_cart)

### Get Cart

Retrieves the latest state of a cart session. Returns `not_found` if the cart does not exist, has expired, or was canceled.

- [REST Binding](https://ucp.dev/specification/cart-rest/#get-cart)
- [MCP Binding](https://ucp.dev/specification/cart-mcp/#get_cart)
...
- [REST Binding](https://ucp.dev/specification/cart-rest/#update-cart)
- [MCP Binding](https://ucp.dev/specification/cart-mcp/#update_cart)
...
- [REST Binding](https://ucp.dev/specification/cart-rest/#cancel-cart)
- [MCP Binding](https://ucp.dev/specification/cart-mcp/#cancel_cart)

## Entities

Cart reuses the same entity schemas as [Checkout](https://ucp.dev/specification/checkout/index.md). This ensures consistent data structures when converting a cart to a checkout session.
...
| Name     | Type                                  | Required | Description                           |
| -------- | ------------------------------------- | -------- | ------------------------------------- |
| item     | [Item](/specification/checkout/#item) | **Yes**  |                                       |
...
| item      | [Item](/specification/checkout/#item)            | **Yes**  |                                                        |
| quantity  | integer                                          | **Yes**  | Quantity of the item being purchased.                  |
| totals    | Array\[[Total](/specification/checkout/#total)\] | **Yes**  | Line item totals breakdown.                            |
| parent_id | string                                           | No       | Parent line item identifier for any nested structures. |
...
This object MUST be one of the following types: [Message Error](/specification/checkout/#message-error), [Message Warning](/specification/checkout/#message-warning), [Message Info](/specification/checkout/#message-info).
...
| code         | [Error Code](/specification/checkout/#error-code) | **Yes**  | Error code identifying the type of error. Standard errors are defined in specification (see examples), and have standardized semantics; freeform codes are permitted.                                                                                                                                                                                                                                                                                                                                          |
