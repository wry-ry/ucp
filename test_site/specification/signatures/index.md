...
This specification defines how to sign and verify UCP messages using [RFC 9421](https://www.rfc-editor.org/rfc/rfc9421) HTTP Message Signatures. For UCP's identity model, supported authentication mechanisms, and key discovery protocol, see [Identity & Authentication](https://ucp.dev/specification/overview/#identity--authentication).
...
Public keys are published in the `signing_keys` array of the party's UCP profile at `/.well-known/ucp`. For the complete key discovery protocol, profile trust model, and profile fetch requirements, see [Identity & Authentication — Key Discovery](https://ucp.dev/specification/overview/#key-discovery).
...
Signature verification errors use standard UCP error codes. See [Error Handling](https://ucp.dev/specification/overview/#error-handling) in the specification overview for the complete error code registry and transport bindings.
