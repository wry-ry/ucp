...
[Tokenization API](https://ucp.dev/handlers/tokenization/openapi.json)
...
The tokenizer **MUST** verify binding matches on `/detokenize`. See
...
one or both depending on your architecture. Or none, like our encrypted payload
...
**When to implement:** Always, unless you combine detokenization with processing
(see PSP example).
...
See the full
[OpenAPI specification](https://ucp.dev/handlers/tokenization/openapi.json) for
...
- [ ] Publishes a handler specification at a stable URL with a unique,
      reverse-DNS `handler_name`
...
- [ ] Requires security acknowledgements from participants receiving raw
      credentials
...
- **[Encrypted Credential Handler](examples/encrypted-credential-handler.md)** —
  Alternative pattern using encryption instead of tokenize/detokenize
  round-trips
- **[AP2 Mandates Extension](ap2-mandates.md)** — Add cryptographic proof of
  checkout agreement for PSP verification
