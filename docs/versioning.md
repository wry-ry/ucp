...
- When the Tech Council approves a new version of UCP, we will cut a new branch
  named `release/YYYY-MM-DD` directly from the current state of `main`.
...
- Timing: We will announce the breaking change in Discussions 2 weeks before the
  change is merged.
