...
def validate_examples(target_file=None):
        ...
        if annotations:
            for match in annotations:
                ...
                if schema_ref == "ignore":
                    print(f"\033[96m[IGNR]\033[0m {location} ignored by annotation")
                    tally["IGNORE"] += 1
                    continue
                ...
                final_json = final_json.replace("{{ ucp_version }}", today_str)
    ...
    print(f"  \033[96mIGNR: {tally['IGNORE']}{COLOR_RESET}")
    print("=" * 30)
