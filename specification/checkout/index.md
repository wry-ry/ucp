...
> **Note:** Checkout permalinks are a REST-specific construct that extends the [REST transport binding](https://ucp.dev/specification/checkout-rest/index.md). Accessing a permalink returns a redirect to the checkout UI or renders the checkout page directly.
...
- [REST Binding](https://ucp.dev/specification/checkout-rest/index.md): RESTful API mapping using standard HTTP verbs and JSON payloads.
- [MCP Binding](https://ucp.dev/specification/checkout-mcp/index.md): Model Context Protocol mapping for agentic interaction.
- [A2A Binding](https://ucp.dev/specification/checkout-a2a/index.md): Agent-to-Agent Protocol mapping for agentic interactions.
...
| totals                    | Array\[[Total Response](/specification/checkout/#total-response)\] | **Yes**  | Fulfillment option totals breakdown.                                       |
...
| Name     | Type                                                                | Required | Description                           |
| -------- | ------------------------------------------------------------------- | -------- | ------------------------------------- |
| item     | [Item Create Request](/specification/checkout/#item-create-request) | **Yes**  |                                       |
...
| --------- | ------------------------------------------------------------------- | -------- | ------------------------------------------------------ |
| id        | string                                                              | No       |                                                        |
| item      | [Item Update Request](/specification/checkout/#item-update-request) | **Yes**  |                                                        |
| quantity  | integer                                                             | **Yes**  | Quantity of the item being purchased.                  |
...
| --------- | ------------------------------------------------------------------ | -------- | ------------------------------------------------------ |
| id        | string                                                             | **Yes**  |                                                        |
| item      | [Item Response](/specification/checkout/#item-response)            | **Yes**  |                                                        |
| quantity  | integer                                                            | **Yes**  | Quantity of the item being purchased.                  |
| totals    | Array\[[Total Response](/specification/checkout/#total-response)\] | **Yes**  | Line item totals breakdown.                            |
| parent_id | string                                                             | No       | Parent line item identifier for any nested structures. |
...
| instruments            | Array\[[Payment Instrument](/specification/checkout/#payment-instrument)\] | No       | The payment instruments available for this payment. Each instrument is associated with a specific handler via the handler_id field. Handlers can extend the base payment_instrument schema to add handler-specific fields. |
...
| Name                   | Type                                                                                   | Required | Description                                                                                                                                                                                                                |
| ---------------------- | -------------------------------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| handlers               | Array\[[Payment Handler Response](/specification/checkout/#payment-handler-response)\] | **Yes**  | Processing configurations that define how payment instruments can be collected. Each handler specifies a tokenization or payment collection strategy.                                                                      |
| selected_instrument_id | string                                                                                 | No       | The id of the currently selected payment instrument from the instruments array. Set by the agent when submitting payment, and echoed back by the merchant in finalized state.                                              |
| instruments            | Array\[[Payment Instrument](/specification/checkout/#payment-instrument)\]             | No       | The payment instruments available for this payment. Each instrument is associated with a specific handler via the handler_id field. Handlers can extend the base payment_instrument schema to add handler-specific fields. |
...
This object MUST be one of the following types: [Card Payment Instrument](/specification/checkout/#card-payment-instrument).
...
| billing_address       | [Postal Address](/specification/checkout/#postal-address)         | No       | The billing address associated with this payment method.                                                                                                           |
| credential            | [Payment Credential](/specification/checkout/#payment-credential) | No       |                                                                                                                                                                    |
...
This object MUST be one of the following types: [Token Credential Response](/specification/checkout/#token-credential-response), [Card Credential](/specification/checkout/#card-credential).
...
| capabilities | Array\[[Response](/specification/checkout/#response)\] | **Yes**  | Active capabilities for this response.     |
